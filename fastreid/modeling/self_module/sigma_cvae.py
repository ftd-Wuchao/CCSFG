...
class sigma_CVAE(nn.Module):
    ...
    def forward(self, x, c, p, aphla):
        ...
        hybrid_loss, log_sigma = self.loss_fn(out_x, x, prior_x, out, prior, aphla)

        return out_x, camids_z, pids_z, prior, hybrid_loss, log_sigma
    ...
    def loss_fn(self, out_x, x, prior_x, out, prior, aphla):
        ...
        log_sigma = ((x - out_x) ** 2).mean([0,1], keepdim=True).sqrt().log()
        log_sigma = torch.tensor(log_sigma.item())

        #mse_loss_fn = nn.MSELoss(reduction='sum').cuda()
        BCE = self.gaussian_nll(out_x, log_sigma,x).sum()
        ...
        L_gsnn = self.gaussian_nll(prior_x,log_sigma, x).sum() / x.size(0)

        hybrid_loss = aphla*L_cvae + (1-aphla)*L_gsnn
        return hybrid_loss, log_sigma.detach()
...
class Encoder(nn.Module):
    ...
    def forward(self, x, c, p):
        c = label2onehot(c, self.camids)
        # c = self.camids_feature(c)
        p = label2onehot(p, self.pids)
        #  p = self.pids_feature(p)
        if self.flag == "camids":
...
class Decoder(nn.Module):
    ...
    def __init__(self, camids, pids, feat_size, latent_size):
        ...
        #layers.append(nn.BatchNorm1d(feat_size))
        self.layers = nn.Sequential(*layers)
